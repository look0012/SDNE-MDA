...
def CNN(x_train,x_test, y_train, y_test):
    ...
    print(x_train.shape)
    print(x_test.shape)
    x_train=x_train.reshape((-1,384,1,1))
    print(x_train.shape)
    x_test = x_test.reshape((-1,384,1,1))
...
while(1):
    ...
    while counter < 5:
        ...
        model1 = CNN(TrainFeature,TestFeature,TrainLabel,TestLabel)

        model = 'model' + str(counter) +'.h5'
        model1.save(model)
        TestFeature=np.array(TestFeature)
        TestFeature = TestFeature.reshape((-1, 384, 1, 1))
        y_score0 = model1.predict(TestFeature)[:,1].tolist()
        predicted1=[]
        for ii in y_score0:
            if ii >= 0.5:
                predicted1.extend('1')
                ...
                predicted1.extend('0')
        y_score0 = list(map(float, predicted1))
        y_score1 = model1.predict_proba(TestFeature)[:,1].tolist()
        ...
        fpr, tpr, thresholds = roc_curve(TestLabel,y_score1)
        fpr = fpr.tolist()
        tpr = tpr.tolist()
        fpr.insert(0, 0)
        tpr.insert(0, 0)
